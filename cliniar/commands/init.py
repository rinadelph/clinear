"""`cliniar init` — scaffold the config file at the standard location."""
...
from cliniar.config import config_path

init_app = typer.Typer(help="Initialize cliniar configuration")


CONFIG_TEMPLATE = """\
# cliniar configuration file
...
# command targets one of these teams (e.g. `cliniar issue get SWA-20`).
# teams = ["SWA", "ENG"]
...
# Detected automatically when you run `cliniar auth workspace`.
# [workspaces]
# "/home/user/work/acme" = "work"
# "/home/user/oss/project" = "personal"

# Named filter aliases. Use with: cliniar issue list --view <name>
...
def init(
    ...
    typer.echo("  2. cliniar me  # verify it works")
    typer.echo("")
    typer.echo("Multi-account setup:")
    typer.echo("  cliniar auth add work --token $LINEAR_WORK_TOKEN")
    typer.echo("  cliniar auth add personal --token $LINEAR_PERSONAL_TOKEN")
    typer.echo("  cliniar auth switch work")
