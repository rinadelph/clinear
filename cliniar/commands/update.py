"""`cliniar update` — check PyPI and upgrade to the latest version."""
...
update_app = typer.Typer(help="Update cliniar to the latest version")


PYPI_JSON_URL = "https://pypi.org/pypi/cliniar/json"
...
def update(
    ...
    """Check PyPI for a newer version and upgrade cliniar."""
    ...
    if manager == "pipx":
        cmd = ["pipx", "upgrade", "cliniar"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "cliniar"]
    ...
    # Human output
    if not needs_update:
        typer.echo(f"cliniar is up to date (v{current})")
        return
...
def _fetch_latest_version() -> str:
    ...
    req = urllib.request.Request(
        PYPI_JSON_URL,
        headers={"Accept": "application/json", "User-Agent": "cliniar/update"},
    )
...
def _detect_package_manager() -> str:
    """Detect whether cliniar was installed via pipx or pip."""
    # Check if pipx is available and manages cliniar
