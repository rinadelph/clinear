"""`cliniar me` and `cliniar auth ...` commands."""
...
def accounts_list() -> None:
    ...
    if not account_rows:
        ...
        typer.echo("Run: cliniar auth add <name> --token <token>")
        return
...
def account_add(
    ...
    if name in config.accounts:
        raise UsageError(
            ...
            hint="Use 'cliniar auth remove {name}' first, or pick a different name.",
        )
...
def account_switch(
    ...
    if name not in config.accounts:
        raise AuthError(
            ...
            hint="Run 'cliniar auth accounts' to see available accounts.",
        )
    config.defaults.default_account = name
    save_config(config)
