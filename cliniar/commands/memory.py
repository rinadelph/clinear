"""`cliniar memory` commands — project-scoped memory board for agents.

The memory board lives at `.cliniar/memory.yaml` (project root). Agents read it
...
from cliniar.errors import NotFoundError
from cliniar.memory_board import (
    MemoryBoard,
    MemoryEntry,
    heal,
    load_memory,
...
memory_app = typer.Typer(help="Project memory board for cliniar agents")
...
def memory_update(
    ...
    if entry is None:
        raise NotFoundError(
            ...
            hint="Run 'cliniar memory list' to see available IDs.",
        )
    ...
    entry.last_updated = __import__("cliniar.memory_board", fromlist=["_now"])._now()
    save_memory(board)
