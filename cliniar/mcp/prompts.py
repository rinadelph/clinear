...
as a user message that walks the agent through running `cliniar` commands in
...
`cliniar` commands to call via the Bash tool.
"""
from __future__ import annotations

REMINDER = (
    "REMINDER: run each command above via the Bash tool. Do not write Python "
    "wrappers around Linear's GraphQL API — `cliniar` is the supported "
...
```bash
cliniar issue list --team {team_key} --no-assignee --state Todo -n 20
```
...
```bash
cliniar issue assign <ID> <USER_NAME>   # or 'me'
cliniar issue update <ID> --priority <N> --label "<labels>"
cliniar issue state <ID> "<NEXT_STATE>"   # e.g. "In Progress"
```
...
```bash
cliniar comment add <ID> --body "Triaged: assigned to <name>, priority <N> because <reason>"
```
...
```bash
cliniar -o md issue list --assignee me --state Done \\
...
```bash
cliniar issue search "{title_hint}" -n 5
```
...
```bash
cliniar comment add <EXISTING_ID> --body "<<<DELIM
{error_text}
DELIM"
```
...
Surface it to the user along with `cliniar issue url <ID>` for a clickable link.
...
```bash
cliniar issue assign {issue_id} "{to_user}"
```

## Step 2 — Move to a review/blocked state

```bash
cliniar issue state {issue_id} "In Review"
```

(Substitute the right state name. Use `cliniar team states <TEAM_KEY>` if you
need to confirm available states for this team.)

## Step 3 — Document the hand-off

```bash
cliniar comment add {issue_id} --body "Handing off to {to_user} — {note_line}"
```
...
```bash
cliniar -o json issue list --cycle current --team {team_key} \\
...
```bash
cliniar issue list --cycle current --team {team_key} --label blocked
```

## Step 4 — Flag stale issues (no update in 5+ days)

```bash
cliniar issue list --cycle current --team {team_key} \\
...
```bash
cliniar -o json issue get {issue_id}
```
...
```bash
cliniar comment list {issue_id} -n 100
```

## Step 3 — Find related issues (same labels)

```bash
LABEL=$(cliniar -o json issue get {issue_id} | jq -r '.labels[0].name // empty')
[ -n "$LABEL" ] && cliniar issue list --label "$LABEL" -n 10
```

## Step 4 — Find related issues (same project / cycle)

```bash
PROJECT=$(cliniar -o json issue get {issue_id} | jq -r '.project.id // empty')
[ -n "$PROJECT" ] && cliniar issue list --project "$PROJECT" -n 10

CYCLE=$(cliniar -o json issue get {issue_id} | jq -r '.cycle.id // empty')
[ -n "$CYCLE" ] && cliniar -o json issue list --cycle "$CYCLE" | jq '[.[] | .identifier]'
```
...
- assign (with `cliniar issue assign`)
- move state (with `cliniar issue state`)
- close as duplicate (set state to `Canceled` and link via comment)
- prioritize (with `cliniar issue prio`)
