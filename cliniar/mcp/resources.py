...
All resources are **read-only**. Mutations live in the `cliniar` CLI — the
...
from cliniar.client import LinearClient
from cliniar.config import load_config, resolve_account, resolve_token
from cliniar.errors import AuthError, CliniarError
from cliniar.graphql import queries
from cliniar.models.cycle import Cycle
from cliniar.models.issue import Issue
from cliniar.models.project import Project
from cliniar.models.team import Team
from cliniar.models.user import User
from cliniar.models.workflow import WorkflowState
...
def _client() -> LinearClient:
    ...
    try:
        ...
        raise RuntimeError(
            "cliniar-mcp could not resolve a Linear API token. "
            "Set $LINEAR_TOKEN, or run `cliniar init` and put your token in "
...
async def viewer() -> str:
    """cliniar://me — currently-authenticated user."""
...
async def issue(id: str) -> str:
    """cliniar://issue/{id} — full Issue including comments, labels, subscribers."""
    async with _client() as c:
...
async def team(key: str) -> str:
    """cliniar://team/{key} — team + states + members rolled into one payload."""
    async with _client() as c:
...
async def project(id_or_slug: str) -> str:
    """cliniar://project/{id_or_slug} — Project with lead + members."""
    async with _client() as c:
...
async def cycle_current(team_key: str) -> str:
    """cliniar://cycle/current/{team_key} — active cycle or {"active_cycle": null}."""
    async with _client() as c:
        # Resolve team id from key
        team_data = await c.execute(
...
async def issues_mine() -> str:
    """cliniar://issues/mine — viewer's open issues (Todo + In Progress + In Review)."""
    async with _client() as c:
...
async def issues_team(team_key: str) -> str:
    """cliniar://issues/team/{team_key} — open issues for a team."""
    async with _client() as c:
        filt = {
