"""Skill content loader + Pydantic model for the `cliniar_guide` MCP tool.

Loads the canonical markdown files from `cliniar/skill_content/*.md` (shipped
...
# ---------------------------------------------------------------------------
DEFAULT_REMINDER = (
    "Run cliniar commands via the Bash tool. Do NOT write Python wrappers "
    "around the Linear GraphQL API — Cliniar already handles auth, retries, "
    ...
    "`cliniar <command>` in a shell."
)


class CliniarGuide(BaseModel):
    """Structured response for the `cliniar_guide` tool.
    ...
    instructions: str = Field(
        description="Markdown instructions teaching the agent how to use Cliniar for this topic.",
    )
...
def _content_dir() -> Path:
    ...
    return Path(str(files("cliniar").joinpath("skill_content")))
...
def _extract_key_commands(markdown: str, limit: int = 10) -> list[str]:
    """Grab the first N `cliniar ...` one-liners that look like bash commands.

    Heuristic: scan fenced ```bash blocks, collect lines starting with `cliniar `.
    ...
    for raw in markdown.splitlines():
        ...
        if not stripped or stripped.startswith("#"):
            continue
        if stripped.startswith("cliniar ") and stripped not in seen:
            seen.add(stripped)
            out.append(stripped)
            if len(out) >= limit:
                return out
...
def load_topic(topic: Topic) -> CliniarGuide:
    """Load a topic's full guide.

    Returns a Pydantic-validated `CliniarGuide` ready to be returned from the
    MCP tool. The MCP SDK will auto-serialize to JSON for the wire.
    """
    body = _read_topic_file(topic)
    title = _extract_title(body, fallback=f"Cliniar — {topic.value}")
    return CliniarGuide(
        topic=topic,
        title=title,
        instructions=body,
