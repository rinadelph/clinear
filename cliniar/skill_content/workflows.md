# cliniar — Workflow Patterns

Multi-step patterns that combine `cliniar` commands. **Always chain via the shell.** Do not write Python wrappers — that's what the CLI exists to avoid.

---

## 1. Triage — claim unassigned issues

When a user says "triage Team ENG" or "clean up the inbox":
...
cliniar issue get ENG-42

# Step 3 — assign + label + (optionally) move state
cliniar issue assign ENG-42 me
cliniar issue update ENG-42 --label "infra"
cliniar issue state ENG-42 "In Progress"
```
...
```bash
cliniar issue list --assignee me --state "In Progress" -n 20
cliniar issue list --assignee me --state "In Review" -n 20
```
...
cliniar issue search "TypeError viewer" -n 5
...
cliniar -o json issue list --cycle current --team ENG | jq 'group_by(.state.name) | map({state: .[0].state.name, count: length, items: map(.identifier)})'
...
cliniar comment add ENG-35 "Handing off to @bob — context: see PR #1234"
```
...
cliniar -o ids issue list --label "old-name" -n 100

# Step 2 — for each id, run the mutation
cliniar -o ids issue list --label "old-name" -n 100 \
    | xargs -I{} cliniar issue update {} --label "new-name"
```
...
```bash
# Full payload
cliniar -o json issue get ENG-35

# Comments
cliniar comment list ENG-35

# Related issues by label
LABEL=$(cliniar -o json issue get ENG-35 | jq -r '.labels[0].name')
cliniar issue list --label "$LABEL" -n 10
```
...
```bash
ISSUE=$(cliniar -o json issue create --team ENG --title "Add retry to API client" --priority 3 | jq -r '.identifier')
echo "Created $ISSUE"
echo "Linear: $(cliniar issue url $ISSUE)"
# Now reference $ISSUE in your PR title or commit message:
# cliniar convention: include the identifier in the branch name (e.g. ENG-35-add-retry)
```
...
```bash
cliniar -o md issue list --assignee me --state "Done" --updated-after $(date -d "7 days ago" +%Y-%m-%d) -n 50
```
...
```bash
ISSUE_ID=$(git log -1 --pretty=%B | grep -oE 'ENG-[0-9]+' | head -1)
if [ -n "$ISSUE_ID" ]; then
    pytest 2>&1 | tail -50 | cliniar comment add "$ISSUE_ID"
fi
```
...
| Don't | Why | Instead |
|---|---|---|
| Write a Python script calling Linear's GraphQL directly | cliniar already handles auth/retries/pagination/validation | Use `cliniar` in the shell |
| Loop in shell when filters would do the job | One API call vs N | Use `cliniar issue list --filter ...` |
| `cliniar issue list \| jq` to filter on team or state | Server-side filters are faster and produce smaller payloads | Use `--team`, `--state`, `--assignee` |
...
| `cliniar issue delete` | Does not exist | Use state transition or archive workflow |
