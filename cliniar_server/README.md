...
```bash
pip install 'cliniar[server]'  # includes the SQLite and psycopg 3 PostgreSQL drivers
```
...
cliniar-serve seed --tenant default
#   -> prints:  TOKEN : lin_api_XXXXXXXX...

# 2. Start the server
cliniar-serve serve --tenant default --port 8787
#   -> http://127.0.0.1:8787/graphql

# 3. Point cliniar at it  (~/.config/cliniar/config.toml)
cat >> ~/.config/cliniar/config.toml <<'TOML'
[accounts.local]
base_url = "http://127.0.0.1:8787/graphql"
token = "lin_api_XXXXXXXX..."      # the token printed by `seed`
TOML

# 4. Use cliniar normally
cliniar --account local me
cliniar --account local issue create --team ENG --title "Hello local"
cliniar --account local issue list
```

`$LINEAR_API_URL` overrides `base_url` for any account, e.g.:

```bash
LINEAR_API_URL=http://127.0.0.1:8787/graphql cliniar me
```
...
| Command | Purpose |
|---------|---------|
| `cliniar-serve migrate` | Apply outstanding ordered schema revisions. |
| `cliniar-serve seed`  | Provision an organization/user/team/states/token. Prints the token. |
| `cliniar-serve serve` | Run the GraphQL server (`--host`, `--port`, `--open`, database flags). |
| `cliniar-serve token` | Mint a token for a selected organization user. |

Every command resolves its database in this order: explicit `--database-url`,
`CLINIAR_DATABASE_URL`, then the existing `--db` or `--tenant` SQLite path.
...
serves many tenants — add several `accounts.<name>` in cliniar config, all with
...
`$XDG_DATA_HOME/cliniar/<tenant>.db` (default `~/.local/share/cliniar/`), WAL mode.
...
and project URL uses `CLINIAR_APP_URL` (default `http://localhost:8787`), so an
...
└── cli.py           `cliniar-serve` entry point (serve|seed|token)
```

## Prove it

```bash
bash scripts/e2e-local-backend.sh      # 17/17 checks: boots server + drives real cliniar CLI
```
